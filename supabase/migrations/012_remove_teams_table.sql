-- 012_remove_teams_table.sql
--
-- Removes the teams table. Category (senior/junior) is now sourced from
-- competitions.category, which already has the right value per league.
-- All stats views are rebuilt to join competitions instead of teams.

-- ── STEP 1: Drop all stats views ─────────────────────────────────────────────
...
WITH innings_totals AS (
  ...
  GROUP BY be.innings_id, be.match_id, mp.player_id, m.season_id, c.category
)
SELECT
  it.player_id,
...
SELECT
  ...
  COUNT(*) FILTER (
    WHERE be.dismissal_type = 'caught'
      AND be.fielder_id = mp.id AND be.fielder_id != be.bowler_id
  )                                                                        AS catches,
  COUNT(*) FILTER (
    WHERE be.dismissal_type = 'caught'
      AND be.fielder_id = mp.id AND be.fielder_id = be.bowler_id
  )                                                                        AS caught_bowled,
  COUNT(*) FILTER (WHERE be.dismissal_type = 'stumped' AND be.fielder_id = mp.id) AS stumpings,
  COUNT(*) FILTER (WHERE be.dismissal_type = 'run_out'  AND be.fielder_id = mp.id) AS run_outs,
  COUNT(*) FILTER (
    WHERE be.dismissal_type IN ('caught','stumped','run_out') AND be.fielder_id = mp.id
  )                                                                        AS total_dismissals
FROM ball_events be
...
JOIN matches m        ON m.id  = be.match_id
JOIN competitions c   ON c.id  = m.competition_id
WHERE mp.player_id IS NOT NULL AND be.fielder_id IS NOT NULL
GROUP BY mp.player_id, p.first_name, p.last_name, m.season_id, c.category;


-- ── STEP 5: career_batting_stats ─────────────────────────────────────────────
...
-- ── STEP 8: competition_batting_stats ────────────────────────────────────────
...
WITH innings_totals AS (
  ...
  JOIN competitions c   ON c.id  = m.competition_id
  WHERE mp.player_id IS NOT NULL
  GROUP BY be.innings_id, be.match_id, mp.player_id, m.competition_id, c.category
)
SELECT
  it.player_id,
  p.first_name || ' ' || p.last_name                                      AS player_name,
...
-- ── STEP 9: competition_bowling_stats ────────────────────────────────────────
...
-- ── STEP 10: competition_fielding_stats ──────────────────────────────────────
...
JOIN competitions c   ON c.id  = m.competition_id
WHERE mp.player_id IS NOT NULL AND be.fielder_id IS NOT NULL
GROUP BY mp.player_id, p.first_name, p.last_name, m.competition_id, c.category;


-- ── STEP 11: Drop team_id from matches, then drop teams ──────────────────────

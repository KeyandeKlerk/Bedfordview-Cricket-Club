-- 008_junior_senior.sql
--
-- Adds category (senior/junior) to the teams table so matches can be
-- filtered by age group.  Also rebuilds all stats views to expose
-- team_category, allowing the stats pages to show junior or senior
-- figures independently.

-- ── STEP 1: Add team_id to matches (may not exist on older installs) ──────────

ALTER TABLE matches
  ADD COLUMN IF NOT EXISTS team_id uuid REFERENCES teams(id);

-- Backfill: assign all existing matches to the first active team (senior default)
-- This is a no-op if team_id is already populated.
UPDATE matches
SET team_id = (SELECT id FROM teams WHERE is_active = true ORDER BY created_at LIMIT 1)
WHERE team_id IS NULL;


-- ── STEP 2: Add category to teams ─────────────────────────────────────────────

ALTER TABLE teams
  ADD COLUMN IF NOT EXISTS category text NOT NULL DEFAULT 'senior'
    CHECK (category IN ('senior', 'junior'));

-- Insert the junior team (idempotent)
INSERT INTO teams (name, short_name, category, is_active)
SELECT 'BCC Junior XI', 'BCCJ', 'junior', true
WHERE NOT EXISTS (SELECT 1 FROM teams WHERE name = 'BCC Junior XI');


-- ── STEP 3: Drop all stats views ──────────────────────────────────────────────
-- Career views depend on season views — drop career first, then season,
-- then competition (independent).
...
-- ── STEP 4: season_batting_stats ──────────────────────────────────────────────

CREATE VIEW season_batting_stats AS
WITH innings_totals AS (
  ...
  GROUP BY be.innings_id, be.match_id, mp.player_id, m.season_id, t.category
)
SELECT
  it.player_id,
...
GROUP BY it.player_id, p.first_name, p.last_name, it.season_id, it.team_category;


-- ── STEP 4: season_bowling_stats ──────────────────────────────────────────────
...
WITH over_summary AS (
  ...
  JOIN teams t           ON t.id  = m.team_id
  WHERE mp.player_id IS NOT NULL
  GROUP BY mp.player_id, m.season_id, t.category, be.innings_id, be.over_number
),
maiden_counts AS (
  SELECT
    player_id,
...
innings_figures AS (
  ...
  JOIN teams t           ON t.id  = m.team_id
  WHERE mp.player_id IS NOT NULL
  GROUP BY mp.player_id, m.season_id, t.category, be.innings_id
),
best_figures AS (
...
LEFT JOIN maiden_counts mc ON mc.player_id    = mp.player_id
                          ...
                          AND mc.team_category = t.category
LEFT JOIN best_figures  bf ON bf.player_id    = mp.player_id
                          AND bf.season_id    = m.season_id
                          AND bf.team_category = t.category
WHERE mp.player_id IS NOT NULL
GROUP BY
  mp.player_id, p.first_name, p.last_name, m.season_id, t.category,
  mc.maidens, bf.best_bowling_wickets, bf.best_bowling_runs;


-- ── STEP 5: season_fielding_stats ─────────────────────────────────────────────

CREATE VIEW season_fielding_stats AS
SELECT
  mp.player_id,
  p.first_name || ' ' || p.last_name                                      AS player_name,
  m.season_id,
...
GROUP BY mp.player_id, p.first_name, p.last_name, m.season_id, t.category;


-- ── STEP 6: career_batting_stats ──────────────────────────────────────────────

CREATE VIEW career_batting_stats AS
SELECT
  player_id,
  player_name,
  team_category,
...
-- ── STEP 8: career_fielding_stats ─────────────────────────────────────────────

CREATE VIEW career_fielding_stats AS
SELECT
  player_id,
  player_name,
  team_category,
...
-- ── STEP 9: competition_batting_stats ─────────────────────────────────────────

CREATE VIEW competition_batting_stats AS
WITH innings_totals AS (
  SELECT
    be.innings_id,
    be.match_id,
  ...
  JOIN teams t          ON t.id  = m.team_id
  WHERE mp.player_id IS NOT NULL
  GROUP BY be.innings_id, be.match_id, mp.player_id, m.competition_id, t.category
)
SELECT
  it.player_id,
  p.first_name || ' ' || p.last_name                                      AS player_name,
...
GROUP BY it.player_id, p.first_name, p.last_name, it.competition_id, it.team_category;


-- ── STEP 10: competition_bowling_stats ────────────────────────────────────────
...
WITH over_summary AS (
  ...
  JOIN teams t           ON t.id  = m.team_id
  WHERE mp.player_id IS NOT NULL
  GROUP BY mp.player_id, m.competition_id, t.category, be.innings_id, be.over_number
),
maiden_counts AS (
  SELECT
    player_id,
...
innings_figures AS (
  ...
  JOIN teams t           ON t.id  = m.team_id
  WHERE mp.player_id IS NOT NULL
  GROUP BY mp.player_id, m.competition_id, t.category, be.innings_id
),
best_figures AS (
...
WHERE mp.player_id IS NOT NULL
GROUP BY
  mp.player_id, p.first_name, p.last_name, m.competition_id, t.category,
  mc.maidens, bf.best_bowling_wickets, bf.best_bowling_runs;


-- ── STEP 11: competition_fielding_stats ───────────────────────────────────────

CREATE VIEW competition_fielding_stats AS
SELECT
  mp.player_id,
  p.first_name || ' ' || p.last_name                                      AS player_name,
  m.competition_id,
...
WHERE mp.player_id IS NOT NULL
  AND be.fielder_id IS NOT NULL
GROUP BY mp.player_id, p.first_name, p.last_name, m.competition_id, t.category;

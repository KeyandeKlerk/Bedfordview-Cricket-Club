...
async function getScorecard(matchId: string) {
  ...
  return { match: { ...match, result_text: resultText }, inningsWithState, playerNameMap }
}
...
export default async function ResultPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { match, inningsWithState, playerNameMap } = data
                        ...
                        {Object.values(state.batterStats).map((b: any) => (
                          <tr key={b.matchPlayerId}>
                            <td>
                              ...
                              <div className="how-out">{b.isOut ? (b.dismissalType?.replace('_', ' ') ?? 'out') : 'not out'}</div>
                            </td>

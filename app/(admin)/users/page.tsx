...
export default function AdminUsersPage() {
  ...
  const [newRole, setNewRole]     = useState<'scorer' | 'admin' | 'shop'>('scorer')
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr auto', gap: 12, alignItems: 'end' }}>
            ...
            <div>
              ...
              <select
                style={{ width: '100%', padding: '8px 12px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 4, color: 'var(--text)', fontSize: 14 }}
                value={newRole}
                onChange={e => setNewRole(e.target.value as 'scorer' | 'admin' | 'shop')}
              >
            ...
            <tbody>
              {roles.map(r => (
                <tr key={r.id}>
                  ...
                  <td><span className={`badge ${r.role === 'admin' ? 'badge-lime' : r.role === 'shop' ? 'badge-sky' : 'badge-gold'}`}>{r.role}</span></td>
